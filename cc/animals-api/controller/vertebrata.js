...
async function getAnimalsByVertebrata(req, res, next) {
    try {
        const results = await new Promise((resolve, reject) => {
            // Example query to retrieve data from a hypothetical 'animal_list' table
            connection.query('SELECT * FROM animal_list WHERE backbone = 1', function(error, results, fields) {
                if (error) {
